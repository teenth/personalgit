 ...
 */
class endView extends mathfat{
    constructor(){
        super();
        this.addEventListener(egret.Event.ADDED_TO_STAGE,this.addBeijing,this)
    }
    //三个按钮的坐标
    private buttonproxy = [{"imgclickname":"clickback","imgnomalname":"nomalback","butx":216,"buty":308,"signresult":"back"},
    ...
    private buttonuntil;
    //记录点了哪个按钮
    private downButtonIndex;
    //粒子系统
    private system:particle.ParticleSystem;
    private lighting;
    private caihong;
    private cloud;
    private huisecaihong;
    ...
    //过关界面
    public passView(type:string,indexss:number,hasmoney:boolean){
        //粒子系统
        this.addParticle();
        //白云弹出缓动动画
        ...
        this.caihong.getMc().addEventListener(egret.Event.COMPLETE,(e:egret.Event)=>{
            //光显示并旋转
            ...
            if(type == "study"){
                ...
                this.addStarAndMoney(false,2);
            }else{
                this.addStarAndMoney(staticVarUntil.iscoin,indexss);
            }
    ...
    //添加星星和钱币
    private addStarAndMoney(showmoney:boolean,indexss:number){
    ...
    //灰色彩虹(失败界面)
    //添加按钮
    private addButton(buttonshanshuoindex:number){
    ...
    //判断点了哪个按钮
    public buttonBegin(evt:egret.TouchEvent) {
    ...
    private getArcProgress(x:number,y:number,r:number) {
        var shape:egret.Shape = new egret.Shape();
        this.addChild(shape);
        var angle:number = 0;
        var times:number = 0;
        egret.startTick(updateChange, this);
        function updateChange(timeStamp:number):boolean {
            angle += 30;
    ...
    //播放声音
    private promptButSound(soundtype:number){
