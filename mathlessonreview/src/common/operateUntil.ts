...
class operateUntil{
    //点击按钮答错的提示
    public static touchButtonWrongMarkWord(times:number,onewrong:string,twowrong:string,threewrong:string){
        if (times == 1) {
    ...
    //asr回答错误
    public static asrWrongMarkWord(times:number,onewrong:string,twowrong:string,threewrong:string){
        if (times == 1) {
            playSoundEffect(onewrong);
        } else if (times == 2) {
            playSoundEffect(twowrong);
    ...
    //触摸响应 obj调用的是哪个对象
    public static onTouchTap(evt:egret.TouchEvent,obj,callback,wrongeffect,rightname){
        ...
        if(butindex != null){
            ...
            if(obj.numarr[butindex].numsign == staticVarUntil.rightresulte){
                ...
                try{callback(obj,butindex);}catch(e){egret.log(e+" callback");}
            }else{
    ...
    //asr回答
    public static asrResult(res:number,rightname,wrongname,callback,obj,rightres){
        //答对了
        if( res == rightres){
            playSoundEffect(rightname);
            try{callback(obj,obj.buttonuntil.allWrongSysShowRight(obj.buttonuntil.butrelatearr,rightres));}catch(e){}
        }else {
            staticVarUntil.everytitlewrongtimes++;
    ...
    //学习或考试结束界面的点击按钮
    public static onTouchTestStudyTap(evt,obj,callback){
        var butindex = obj.buttonuntil.findButtonIndex(obj.buttonuntil.butrelatearr,evt);
        if(butindex != null){
    ...
    //根据正确答案是多少提示相应的语言
    public static sayWordByRightResult(num:number){
        if(num == 1){
            return "one";
        }else if(num == 2){
            return "two";
    ...
    public static getScore(num:number =1){
        if(staticVarUntil.numbersign.indexOf("english")) num =2;
    ...
    //初始化得分
    public static initTotalWrong(){
        staticVarUntil.totalwrong = 0;
    }
    //弹星界面启动触摸 学习完以后的结算界面启动触摸
    public static endViewOpenTouch(){
        viewManager.getInstance().touchEnabled = true;
    }
    //回答我不知道 或我不会直接给出正确答案
    public static answerNoGiveRightResult(){
...
//        operateUntil.initScore();
    ...
    public static pauseNumber(){
        ...
        if(staticVarUntil.numbersign == staticVarUntil.MATH_1_UNIT_1_NUMBERONE){
            ...
            //viewManager.getInstance().removeChildren();
    ...
    //取消锚点设置
    public static cancelAnchorOffsetxy(obj){
    ...
    //恢复锚点设置(锚点在对象的中间)
    public static restoreAnchorOffsetxy(obj,width:number=obj.width/2,height:number=obj.height/2){
